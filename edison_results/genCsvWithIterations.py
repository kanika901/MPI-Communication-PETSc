...
def getCSV(thedata, noOfIterations):
    csvlines = []
    buf = 'config,nprocs,solverid,solvername,' + ','.join(keys) + 'no. of iterations' + '\n' #setting csv header
    for conf,pdata in thedata.items():
      for nproc,v in pdata.items():
        for solverid,vv in v.items():
            for function in vv.values():
                if function[0] == 'KSPSolve':
                    csvlines.append(str(conf) + ',' + str(nproc) + ',' + str(solverid) + ',' + solvers[solverid] + ',' + ','.join(function) + ',' + noOfIterations[solverid])
...
if __name__ == "__main__":
    ...
    for f in files:
        ...
        for l in lines:
            ...
            if l.startswith('Number of SNES iterations'):
                print(l, f)
                noOfIterations[f.split('.')[1]] = l.split()[-1]
    ...
    f = open("edison-ex19-kspsolve_v2.csv","w")
    #print(data)
    f.write(getCSV(data, noOfIterations))
    f.close()

    f = open("edison-ex19-kspsolve-bad_v2.csv","w")
