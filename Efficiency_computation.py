...
with open('Scalability.csv', 'r+') as csvinput:
		...
		with open(csvoutput,'w') as csvoutput:
			...
			num_procs = []
			writer.writerow(header + ['speedup'] + ['efficiency']) #write the header to the new decoupled file before writing the feature values
			for row in islice(infile, 1, None):
				...
				speedup = solver_details.base_time/time  #1
